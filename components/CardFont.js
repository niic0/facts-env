...
const CardFront = ({fact }) => {
    ...
    return (
        <div className="relative mx-4 rounded-2xl bg-white drop-shadow-lg px-4 py-7 my-auto w-96 h-[600px]">
            <div className="font-bold text-2xl mb-8 text-neutral-700">
                {fact.fact}
            </div>

            <div className="flex items-center justify-around absolute bottom-5 left-0 right-0">
                <button onClick={() => swiper.slideNext()} className="rounded-full bg-neutral-700 p-4">
                ...
                <button onClick={() => swiper.slideNext()} className="rounded-full bg-neutral-700 p-4 hover:bg-neutral-600 transition">
                    <svg width="29" height="29" viewBox="0 0 36 32" fill="none" xmlns="http://www.w3.org/2000/svg">
